...
public class Stream8 {
    ...
    public static List<String> getFirstNames(List<String> names){
        //return names.stream().map(n -> n.substring(0, n.indexOf(" "))).collect(toList());
    ...
    public static Integer getMaxAge(List<User> users){
        return users.stream().map(User::getAge).max(Comparator.comparing(Function.identity())).orElse(-1);
    }
    ...
    public static boolean anyMatch(List<User> users, int age){
        //return users.stream().map(User::getAge).anyMatch(u -> u == age);
